...
function MentorComment({ mentorData, isTouchable = true }) {
  ...
  const TAG = 'MentorComment';
  const htmlData = JSON.parse(mentorData?.content);

  // generating intials of name
  let acronym = '';
  if (mentorData) {
      ...
      <View style={Styles.answerContainerStyle}>
        ...
        <View style={Styles.ansWebViewStyle}>
          <View style={Styles.renderHtmlContainer}>
            <RenderHTML
              baseStyle={{ color: appTheme.TEXT_COLOR_HEADING }}
              contentWidth={width}
